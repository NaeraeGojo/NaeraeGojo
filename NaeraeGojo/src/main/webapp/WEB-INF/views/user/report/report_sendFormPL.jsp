...
// 			var pw_code = $("#pwList1 option:selected").val();
			...
			$('#reportForm').attr('action','${pageContext.request.contextPath }/user/report/report_FinalInsertPL.do');
			$('#reportForm').submit();
			return true;
		});
...
// 			var pw_code = $("#pwList1 option:selected").val();
			...
			$('#reportForm').attr('action','${pageContext.request.contextPath }/user/report/report_FinalInsertPLSave.do');
			$('#reportForm').submit();
			return true;
		});
	...
	function change(value){
			...
			$.ajax({
				url : '${pageContext.request.contextPath}/user/report/report_pw_peoplePL.do?project_code='+ value,
				dataType: 'json',
				success : function(data){
					console.log(data);
					info = data.proPeoplePM;
					var code ='';
					$('#bodytable').empty();
						code += '<tr>';
						code += '<input type="hidden" name="emp_code" value="'+ info.emp_code+'">';
						code += '<td>'+info.emp_department+'</td>';
						code += '<td>'+info.emp_level+'</td>';
						code += '<td>'+info.emp_name+'</td>';
						code += '<td>'+info.position_name+'</td>';
						code += '</tr>';
